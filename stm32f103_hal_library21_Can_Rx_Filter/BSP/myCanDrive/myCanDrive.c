...
static void CAN_FilterConfig_AllMessages(CAN_HandleTypeDef *hcan)
{
    ...
    // 选择滤波器号（根据实际需求，如果只有一个CAN，一般可用0号滤波器）
...
/**
  ...
  */
static void CAN_FilterConfig_Bank2_RangeStd(CAN_HandleTypeDef *hcan)
{
    CAN_FilterTypeDef filter;

    filter.FilterBank           = 2;                        // 选择 Bank2
    filter.FilterMode           = CAN_FILTERMODE_IDMASK;    // Mask 模式
    filter.FilterScale          = CAN_FILTERSCALE_16BIT;    // 16-bit × 4

    /*—— 16-bit 编码 ——*/
    /* 标准 ID 左移 5 位；IDE=0、RTR=0 */
    /* 0x200 → 0x4000，0x300 → 0x6000                              */
    /* 掩码 FE0C：忽略 ID[3:0]，但强制 IDE/RTR=0（标准数据帧）      */

    /* 组1：0x200~0x20F */
    filter.FilterIdHigh         = 0x4000;   // ID1
    filter.FilterMaskIdHigh     = 0xFE0C;   // MASK1

    /* 组2：0x300~0x30F */
    filter.FilterIdLow          = 0x6000;   // ID2
    filter.FilterMaskIdLow      = 0xFE0C;   // MASK2

    filter.FilterFIFOAssignment = CAN_RX_FIFO1;             // 进 FIFO1
...
/**
  * @brief  Filter Bank1，List-16bit，接收 0x58 / 0x158 / 0x258 / 0x358 → FIFO1
  */
static void CAN_FilterConfig_Bank1_ListStd(CAN_HandleTypeDef *hcan)
{
    CAN_FilterTypeDef filter;

    filter.FilterBank           = 1;                       // Bank 1
    filter.FilterMode           = CAN_FILTERMODE_IDLIST;   // List 模式
    filter.FilterScale          = CAN_FILTERSCALE_16BIT;   // 16-bit ×4

    /* 四个离散标准帧的 16-bit 编码值 */
    filter.FilterIdHigh         = 0x0B00;   // ID0  : 0x058
    filter.FilterIdLow          = 0x2B00;   // ID1  : 0x158
    filter.FilterMaskIdHigh     = 0x4B00;   // ID2  : 0x258
    filter.FilterMaskIdLow      = 0x6B00;   // ID3  : 0x358
...
void CAN_Config(void)
{
    ...
    CAN_FilterConfig_Bank2_RangeStd(&hcan); // 配置过滤器2，标准帧，接收CANID(0x200~0x20F、0x300~0x30F)
    CAN_FilterConfig_Bank1_ListStd(&hcan);  // 配置过滤器1，标准帧，接收CANID(0x58,0x158,0x258,0x358) 

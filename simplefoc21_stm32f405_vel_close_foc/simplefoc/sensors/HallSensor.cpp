...
void HallSensor::handleA() {
  ...
  A_active = HAL_GPIO_ReadPin(M0_ENC_A_GPIO_Port, M0_ENC_A_Pin);
  updateState();
}
// B channel
void HallSensor::handleB() {
  //B_active = digitalRead(pinB);
  B_active = HAL_GPIO_ReadPin(M0_ENC_B_GPIO_Port, M0_ENC_B_Pin);
  updateState();
}
...
void HallSensor::handleC() {
  ...
  C_active = HAL_GPIO_ReadPin(M0_ENC_C_GPIO_Port, M0_ENC_C_Pin);
  updateState();
}
...
//  if(doC != nullptr) attachInterrupt(digitalPinToInterrupt(pinC), doC, CHANGE);
  ...
  //! 我们在CubeMX勾选了中断使能，所以，HAL库的gpio.c已经自动打开了中断。
}

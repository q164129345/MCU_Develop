...
void main_Cpp(void)
{
    ...
    motorDriver.enable();
    HAL_Delay(10);
    currentSense.init();  // 初始化电流传感器，检测偏置电压，也就是电流0A时的运放输出电压值，理论值=1.65V
    HAL_Delay(10);
    motorDriver.disable();
    //!
		motor.linkSensor(&sensor); // 将霍尔传感器跟电机连接起来
		motor.linkDriver(&motorDriver); // 将电机驱动器与电机对象关联
		motor.linkCurrentSense(&currentSense); // 关联电流传感器
								
    // 速度环
    motor.PID_velocity.P = 0.1f;  // 设置速度P
    motor.PID_velocity.I = 2.5f;  // 设置速度I
    motor.PID_velocity.D = 0;     // 设置速度D
    motor.PID_velocity.output_ramp = 800; // 设置速度斜坡
    motor.LPF_velocity.Tf = 0.01f; // 设置速度低通滤波器
    // 电流环q轴(控制扭矩)
    motor.PID_current_q.P = 0.15f;
    motor.PID_current_q.I = 8.0f;
    motor.PID_current_q.D = 0.0f;
    motor.PID_current_q.output_ramp = 0; // 设置电流斜坡
    motor.LPF_current_q.Tf = 0.01f; // 设置电流q轴低通滤波器
    // 电流环d轴(控制磁性)
    motor.PID_current_d.P = 0.1f;
    motor.PID_current_d.I = 1.0f;
    motor.PID_current_d.D = 0.0f;
    motor.PID_current_d.output_ramp = 0; // 设置电流斜坡
    motor.LPF_current_d.Tf = 0.01f; // 设置电流d轴低通滤波器
    ...
    motor.sensor_direction = Direction::CCW; // 测量传感器方向

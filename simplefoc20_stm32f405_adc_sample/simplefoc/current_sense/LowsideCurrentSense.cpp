...
void LowsideCurrentSense::calibrateOffsets(){    
    ...
    for (int i = 0; i < calibration_rounds; i++) {
        ...
        if(_isset(pinA)) offset_ia += (_readADCVoltageLowSide(pinA));
        if(_isset(pinB)) offset_ib += (_readADCVoltageLowSide(pinB));
...
PhaseCurrent_s LowsideCurrentSense::getPhaseCurrents(){
    ...
    //_startADC3PinConversionLowSide();
    current.a = (!_isset(pinA)) ? 0 : (_readADCVoltageLowSide(pinA) - offset_ia)*gain_a;// amps
    current.b = (!_isset(pinB)) ? 0 : (_readADCVoltageLowSide(pinB) - offset_ib)*gain_b;// amps

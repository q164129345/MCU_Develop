...
OP_FlashStatus_t OP_Flash_Copy(uint32_t src_addr, uint32_t dest_addr, uint32_t length)
{
    ...
    while (remaining > 0) {
        //! 4. 计算本次要搬运的数据块大小
        //    取 剩余量 和 缓冲区大小 中的较小者。
        uint32_t chunk_size = (remaining > FLASH_COPY_BUFSIZE) ? FLASH_COPY_BUFSIZE : remaining;

        //! 5. 计算当前块在源地址和目标地址的位置
        //    从末尾向前计算
        uint32_t current_src_addr = src_addr + remaining - chunk_size;
        uint32_t current_dest_addr = dest_addr + remaining - chunk_size;

        //! 6. 从源地址读取一个数据块到缓冲区
        memcpy(buffer, (const void*)current_src_addr, chunk_size);

        //! 7. 将数据块写入目标地址
        if (OP_Flash_Write(current_dest_addr, buffer, chunk_size) != OP_FLASH_OK) {
            return OP_FLASH_ERROR;
        }

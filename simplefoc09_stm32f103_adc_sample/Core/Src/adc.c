...
/* USER CODE BEGIN 0 */
// 当变量多了，后续可以用struct组织起来
static volatile uint32_t gChannel3 = 0; // A相
static volatile uint32_t gChannel4 = 0; // B相
/* USER CODE END 0 */
...
/**
  ...
  */
static HAL_StatusTypeDef ADC_ReadVoltageFromChannels(ADC_HandleTypeDef *hadc)
{
    ...
    status = HAL_ADC_PollForConversion(hadc, 10); // 超时10ms，可根据需求调整
    if (status == HAL_OK) {
        gChannel3 = HAL_ADC_GetValue(hadc); // 读取第1个转换结果
    } else {
        // 读取失败，记得停止ADC
        HAL_ADC_Stop(hadc);
        return status;
    ...
    if (status == HAL_OK) {
        gChannel4 = HAL_ADC_GetValue(hadc); // 读取第2个转换结果
    } else {
        HAL_ADC_Stop(hadc);
        return status;
    }

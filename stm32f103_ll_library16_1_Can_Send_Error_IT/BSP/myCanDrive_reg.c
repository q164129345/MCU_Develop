...
CAN_ESR_t gCanESR = {0,};
volatile uint8_t g_ErrorEventCanBusoff = 0;
volatile uint8_t g_ErrorCountCanBusoff = 0;
/**
...
void CAN_Config(void)
{
    ...
    */
    ...
    NVIC_SetPriority(CAN1_SCE_IRQn, NVIC_EncodePriority(NVIC_GetPriorityGrouping(),4, 0));
    NVIC_EnableIRQ(CAN1_SCE_IRQn);
    CAN1->IER |= (CAN_IER_BOFIE | CAN_IER_ERRIE); // 启用错误警告、错误被动及总线关闭中断
...
/**
 * @brief  将寄存器CAN_ESR的各个字段解析，赋值给全局结构体变量gCanESR，方便使用。
 * @note   
 */
void CAN_Read_CAN_Error(void)
{
    uint32_t esr = CAN1->ESR;
    /* 解析所有错误字段 */
    gCanESR.lec   = (esr & CAN_ESR_LEC)   >> CAN_ESR_LEC_Pos;   // 最后错误代码（Last Error Code）

...
extern volatile uint8_t g_ErrorCountCanBusoff;
...
int main(void)
{
  ...
  */
  ...
  while (1)
  {
    ...
    /* 监控CAN错误 */
    if (g_ErrorEventCanBusoff) { // 发生CAN离线错误事件
        g_ErrorEventCanBusoff = 0; // 清除标志
        CAN_Read_CAN_Error(); // 解析CAN_ESR寄存器的错误
        g_ErrorCountCanBusoff++; // 每发生一次busoff严重错误，记录一次
